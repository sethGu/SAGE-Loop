...
def stacking_ensemble(base_models, meta_model, X, y, X_test, y_test, n_folds=5, verbose=True, random_state=42, top_k=5):
    """
    执行 Stacking 集成学习，自动筛选 AUC 表现最好的 base_models。
    ...
    # Step 0: 单独评估每个 base_model 的 AUC，选 top_k
    auc_scores = []
    for i, model in enumerate(base_models):
        try:
            model.fit(X_train, y_train)
            probas = model.predict_proba(X_test)
            if len(probas.shape) == 1 or probas.shape[1] == 1:
                score = roc_auc_score(y_test, probas.ravel())
            else:
    ...
    # 选出 top_k 模型索引
    top_indices = np.argsort(auc_scores)[-top_k:][::-1]
    selected_models = [base_models[i] for i in top_indices]

    if verbose:
        print(f"\n✅ Selected top-{top_k} base models based on AUC:")
        for i in top_indices:
            print(f"  - Model {i}: AUC = {auc_scores[i]:.4f}")
    ...
    for model_idx, model in enumerate(selected_models):
        ...
        for fold_idx, (train_idx, val_idx) in enumerate(skf.split(X_train, y_train)):
            ...
            if len(val_pred_proba.shape) == 1 or val_pred_proba.shape[1] == 1:
                train_meta[val_idx, model_idx] = val_pred_proba.ravel()
            else:
                train_meta[val_idx, model_idx] = val_pred_proba[:, 1]
    ...
    for model_idx, model in enumerate(selected_models):
        ...
        if len(test_pred_proba.shape) == 1 or test_pred_proba.shape[1] == 1:
            test_meta[:, model_idx] = test_pred_proba.ravel()
        else:
            test_meta[:, model_idx] = test_pred_proba[:, 1]
